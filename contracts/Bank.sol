...
contract Bank is AccessControl, Pausable, ReentrancyGuard {
     ...
     * */
    bytes32 public constant BANK_OPERATOR_ROLE =
        keccak256("BANK_OPERATOR_ROLE");
    bytes32 public constant OPERATOR_MANAGER_ROLE =
        keccak256("OPERATOR_MANAGER_ROLE");
    bytes32 public constant CONFIG_MANAGER_ROLE =
...
*/
    constructor(
        ...
        uint _interestRatePerAnnum,
        uint _depositFeePercent,
        uint _dustThreshold
    ) {
        require(
            _feeCollectorAddress.code.length > 0,
            InputNotContractAddress()
        );
        require(_feeCollectorAddress != address(0), InvalidAddress());
        require(
            _interestRatePerAnnum >= 1 && _interestRatePerAnnum <= 100,
            NotWithinValidRange()
        );
        require(
            _depositFeePercent >= 1 && _depositFeePercent <= 50,
            NotWithinValidRange()
        );
     ...
     * */
    function setNewFeeCollectorAddress(
        address _newFeeCollectorAddress
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _newFeeCollectorAddress.code.length > 0,
            InputNotContractAddress()
        );
        ...
        emit NewFeeCollectorContractSet(
            _newFeeCollectorAddress,
            block.timestamp
        );
    }
     ...
     */
    function setNewDustThreshold(
        uint newDustThreshold
    ) external onlyRole(BANK_CONFIG_ROLE) {
     ...
     */
    function setNewDepositFeePercent(
        uint _newDepositFeePercent
    ) external onlyRole(BANK_CONFIG_ROLE) {
        require(
            _newDepositFeePercent >= 1 && _newDepositFeePercent <= 50,
            NotWithinValidRange()
        );
        require(
     ...
     */
    function setNewInterestRate(
        uint _newInterestRatePerAnnum
    ) external onlyRole(BANK_CONFIG_ROLE) {
        require(
            _newInterestRatePerAnnum >= 1 && _newInterestRatePerAnnum <= 100,
            NotWithinValidRange()
        );
        require(
    ...
    function deposit() external payable whenNotPaused {
        ...
        if (depositBalance[msg.sender] > 0) {
            ...
            depositTimeStamp[msg.sender] =
                ((depositTimeStamp[msg.sender] * oldBalance) +
                    (block.timestamp * newDeposit)) /
                (totalBalance);
        ...
        emit Deposited(
            ...
            totalInterest[msg.sender]
        ); //emit logs the transactions tracking who deposited money and amount
    }
    ...
    function calculateInterest(address user) internal view returns (uint) {
        ...
        uint interest = (depositBalance[user] *
            (interestRatePerAnnum) *
            secondsPassed) / (100 * 365 * 86400); //interest = PRT/100
        return interest;
    }
 ...
 */
    function withdraw(uint _amount) external whenNotPaused nonReentrant {
        ...
        require(
            depositBalance[msg.sender] >= DUST_THRESHOLD,
            InsignificantBalance()
        );
    ...
    function withdrawAll() external whenNotPaused nonReentrant {
        ...
        require(
            address(this).balance >= depositBalance[msg.sender],
    ...
    function checkUserBalance(
        address user
    ) external view onlyRole(BANK_OPERATOR_ROLE) returns (uint) {
